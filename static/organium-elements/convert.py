...
def copy_thumbnail(component_name):
    """Return the relative path of the thumbnail for use as an index card thumbnail.
    Prefers the compressed version in assets/img/thumbs/ (generated by generate_thumbs.py).
    Falls back to the full-size image if no thumb exists yet."""
    src = find_thumbnail(component_name)
    if not src:
        return None
    slug_name = os.path.splitext(os.path.basename(src))[0]
    thumb_path = os.path.join(ASSETS_IMG, 'thumbs', slug_name + '.jpg')
    if os.path.exists(thumb_path):
        return f"assets/img/thumbs/{slug_name}.jpg"
